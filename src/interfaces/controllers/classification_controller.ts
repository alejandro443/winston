...
export class ClassificationController {
  ...
  @HttpCode(201)
  @Get('/one/:code')
  async getOneClassification(@Param() request: GetClassificationRequestDto): Promise<AppResponse> {
    Log.info(`(Get) Get classification code: ${request.code}`)

    const classification = await this.application.getOneClassification(request.code)
    return {
      status: 201,
  ...
  @HttpCode(201)
  @Post()
  async createClassification(@Body() request: CreateClassificationRequestDto): Promise<AppResponse> {
  ...
  @HttpCode(200)
  @Put('/update/:code')
  async updateClassification(@Param() params: GetClassificationRequestDto, @Body() request: CreateClassificationRequestDto): Promise<AppResponse> {
    Log.info(`(PUT) Put classification`)

    const classification = await this.application.updateClassification(params.code, request)
    return {
      status: 200,
  ...
  @HttpCode(200)
  @Delete('/delete/:code')
  async deleteClassification(@Param() params: GetClassificationRequestDto): Promise<AppResponse> {
    Log.info(`(Delete) Delete classification ${params.code}`)

    const classification = await this.application.deleteClassification(params.code)
    return {
      status: 200,
