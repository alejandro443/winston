...
export class TypeDocumentController {
  ...
  @HttpCode(201)
  @Get('/one/:code')
  async getOneTypeDocument(@Param() request: GetTypeDocumentRequestDto): Promise<AppResponse> {
    Log.info(`(Get) Get type_document code: ${request.code}`)

    const type_document = await this.application.getOneTypeDocument(request.code)
    return {
      status: 201,
  ...
  @HttpCode(201)
  @Post()
  async createTypeDocument(@Body() request: CreateTypeDocumentRequestDto): Promise<AppResponse> {
  ...
  @HttpCode(200)
  @Put('/update/:code')
  async updateTypeDocument(@Param() params: GetTypeDocumentRequestDto, @Body() request: CreateTypeDocumentRequestDto): Promise<AppResponse> {
    Log.info(`(PUT) Put type_document`)

    const type_document = await this.application.updateTypeDocument(params.code, request)
    return {
      status: 200,
  ...
  @HttpCode(200)
  @Delete('/delete/:code')
  async deleteTypeDocument(@Param() params: GetTypeDocumentRequestDto): Promise<AppResponse> {
    Log.info(`(Delete) Delete type_document ${params.code}`)

    const type_document = await this.application.deleteTypeDocument(params.code)
    return {
      status: 200,
